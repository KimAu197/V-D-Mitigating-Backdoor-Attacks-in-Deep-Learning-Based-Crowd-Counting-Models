...
if __name__=="__main__":
    ...
    checkpoint_file = "/root/autodl-tmp/checkpoint/checkpoints/100_add.pth"
    ...
    min_mae = sys.maxsize
    min_mae_epoch = -1
    for epoch in range(cfg.epochs):                          # start training
        model.train()
        epoch_loss = 0.0
        ...
        if epoch % 20 == 0:
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(tqdm(test_dataloader)):
                    ...
                    epoch_mae += mae.item()
                    cfg.writer.add_text("record Text:", str(epoch) + str(i) + "estimate:" + str(et_densitymap.data.sum()) +  "truth" + str(gt_densitymap.data.sum()) )
                ...
                if epoch_mae < min_mae:
                    ...
                    torch.save(model.state_dict(), "./checkpoints/100_add_ft.pth")
                ...
                # text = []
                # for i in range(len(et_densitymap)):
                #     every = str(epoch)+'/Estimate density count:'+ str('%.2f'%(et_densitymap[i].cpu().sum())) +'/Ground Truth count:'+ str('%.2f'%(gt_densitymap[i].cpu().sum())) + "\n"
                #     text.append(every)
                # every_string = ' '.join(text)cd ..
                # cfg.writer.add_text("record Text",every_string)
