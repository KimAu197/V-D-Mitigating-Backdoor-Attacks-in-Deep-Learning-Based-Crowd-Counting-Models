...
from datasets import ShbTest, Resize2Multiple
...
def main(args: ArgumentParser):
    print("Testing a trained model on the Shb-Crowd test set.")
    ...
    dataset = ShbTest(transforms=transforms, return_filename=True)

    image_ids = []
    preds = []
    

    for idx in tqdm(range(len(dataset)), desc="Testing on SHB"):
        ...
        # save_image(preds[0]/torch.max(preds[0]), os.path.join("./test_data/", "/root/autodl-tmp/CLIP-EBC-main/Shb_test_results/image", "/et_densitymap{}.jpg".format(torch.Tensor(idx))))

    result_dir = os.path.join(current_dir, "Shb_test_results/output3")
    os.makedirs(result_dir, exist_ok=True)
    weights_dir, weights_name = os.path.split(args.weight_path)
    model_name = os.path.split(weights_dir)[-1]
    result_path = os.path.join(result_dir, "data_min100.csv")
    
    gts = []
    for i in range(1,317):
        mat_path = os.path.join("/root/autodl-tmp/CLIP-EBC-main/data/ShanghaiTech/part_B/test_data/ground-truth", "GT_IMG_{}.mat".format(i))
    ...
    with open(result_path, "w", newline='') as csvfile:
        ...
        csvwriter.writerow(["ImageID", "gt", "Prediction", "rate"])
        for image_id, pred, gt in zip(image_ids, preds, gts):
            csvwriter.writerow([image_id, gt, pred, pred/gt])
